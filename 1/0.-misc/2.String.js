// --------------------------------------
// Exercise 3 - Add numbers from string to float

const numberOne = "1.10";
const numberTwo = "2.30";

// add those two numbers and show the result
// you cannot touch line 1 neither line 2

const numberThree = parseFloat(numberOne);
const numberFour = parseFloat(numberTwo)
console.log(numberThree + numberFour)

const result = parseFloat(numberOne) + parseFloat(numberTwo);
console.log(result);

// --------------------------------------


// --------------------------------------
// Exercise 4 - Add the numbers and the total with 2 decimals

const anotherNumberOne = "1.10";
const anotherNumberTwo = "2.30";

const solution = parseFloat(anotherNumberOne) + parseFloat(anotherNumberTwo);
console.log(solution.toFixed(2));

// --------------------------------------
// Exercise 5 - Decimals and average

const one = 10;
const two = 45;
const three = 98;

// Show in the console the avg. with 5 decimals
// make variable
const resultTwo = (one +two +three) / 3;
console.log(resultTwo.toFixed(5));





// --------------------------------------
// Exercise 6 - Get the character by index

const letters = "abc";
// Get me the character "c"

// makes an array of 'letters' and outputs it 
console.log(letters[2]);

//outputs index of C
console.log(letters.indexOf('c'));



// --------------------------------------
// Exercise 7 - Replace

const fact = "You are learning javascript!";

// capitalize the J in Javascript

const substringOne = fact.slice(0,16)
const subStringTwo = fact[17].toUpperCase()+fact.slice(18)

//Opdeler string i to substrings og sætter dem samme. 
//console.log(fact.slice(0,16)+" "+fact[17].toUpperCase()+fact.slice(18));
//console.log(fact[17].toUpperCase()+fact.slice(18));


// --------------------------------------



